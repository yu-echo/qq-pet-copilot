...
- **被雇佣检查 / 召回**：按时间段定时出门检测被雇佣中，按配置「等到 25/75」或「立刻召回」处理，
  召回自动计数后回主页面；被雇佣期间主任务不触发。雇佣自然到期（面板消失）时不会卡住等待，
  自动判定结束并继续调度。
...
- **💰 成长福袋领取**：自动点取好友列表里的成长福袋，
  推荐区域用「他们都在玩」分界线排除，翻页有上限与时间预算；
  领到的金币累计显示在今日统计的「福袋」格（换宠物或配置变更后自动清零重计）。
- **🎁 SVIP 会员礼包**：每天到点自动领取一次 QQ SVIP 会员礼包，
  自动区分「立即领取 / 明日再来 / 开通 SVIP」三种弹窗状态；非会员会识别出来并自动关闭
  该任务，关闭期间每天复查一次会员状态，恢复会员时自动重新开启。
...
- **GUI（PyQt6-Fluent-Widgets）**：左侧 Fluent 导航（主页/调度/统计/任务/设置），顶部全局
  ...
  `gui.mirror`）；主页 = scrcpy 实时画面（9:16 竖屏等比自适应嵌入，前后台切换时镜像常驻不重启，
  避免手机屏幕闪烁）+ 宠物状态/任务队列/今日统计/日志卡片；
  宠物状态卡标题右侧显示当前**账号名称 / 宠物名称**，换宠物时自动重置全部每日任务进度——任务队列卡调度器未运行时也按配置推算下一任务，今日统计以
